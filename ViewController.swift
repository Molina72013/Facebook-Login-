...
//                        print(url)
    ...
    @IBAction func customLogin(_ sender: UIButton)
    {
        //CHECK FOR USER
        if FBSDKAccessToken.current() == nil
        {
    ...
    @IBAction func customShare(_ sender: UIButton)
    {
        //CHECK FOR USER
        if FBSDKAccessToken.current() != nil
        {
        let photo = FBSDKSharePhoto()
        photo.image = UIImage(named: "lambo")
        photo.isUserGenerated = true
        let content = FBSDKSharePhotoContent()
        content.photos = [photo]
    ...
    private func loadImageRequest(url: String)
    {
